...
- **Presentation layer** lives in `src/features/desktop/components/` + `src/features/desktop/hooks/`, consuming the window manager via `useWindowManager` and the start menu via `useStartMenu`:
  ...
  - `Taskbar.tsx` — fixed bottom bar with a Start button (toggles the start menu and gets the `active` class while it is open), one button per window sorted by ascending z-index (showing the registered app icon; focused window gets the `focused` class and a persistent pressed shadow), and a `Clock`. Taskbar-button clicks dispatch the action returned by `resolveTaskbarAction`.
