...
- **App registry & routing** (`src/apps/`, a top-level module alongside `src/features/`) wires apps into the desktop and the URL:
  - `registry.ts` / `index.ts` — `appRegistry: Record<AppId, AppDescriptor>` enumerating the apps (`computer`, `about`, `contact`, `cv`, `projects`, `terminal`). Each entry carries its `IconName`, `defaultSize`, `resizable` / `singleton` flags, and a `component: React.lazy(...)` for code-splitting. `About` / `CV` / `Projects` / `Contact` / `Computer` are singletons; `Terminal` allows multiple instances. Real app content lives under `src/apps/<app>/` — `src/apps/about/AboutApp.tsx` is the first (rendered for the resizable `about` entry) — while the other apps still use `PlaceholderApp.tsx` until their content lands.
